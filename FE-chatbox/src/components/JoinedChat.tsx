...
export function JoinedChat() {
  ...
  async function fetcher() {
    const response = await axios.get(
      `${import.meta.env.VITE_API_URL}/room/fetcher`,
      {
  ...
  useEffect(() => {
    ...
    const messageHandler = (e: MessageEvent) => {
      ...
      if(data.type==="joiners-resposnse"){
        const msgs = data.payload;
        if(msgs && typeof msgs === "object"){
          setConnected(msgs.connected);
        }
  ...
  return (
    <div className="bg-black h-full w-screen flex justify-center items-center box-border">
      {localStorage.getItem("token") ? (
        <div className="border-box border-2 border-gray-800 p-6 flex flex-col gap-y-4 rounded-md w-full max-w-md font-mono text-white mt-7 mb-7">
